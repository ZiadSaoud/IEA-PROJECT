		...
		createEnv.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				if (height == 0 || width == 0) {
					...
					Line l4 = drawLine(800, 750, bx + width * 50 + (width - 1) * 4,
							by + height * 50 + (height - 1) * 4);
					...
					for (int i = 0; i < height; i++) {
						for (int j = 0; j < width; j++) {
							gridPane.getChildren()
									.add(tiles[i][j] = new tile(j * 54 + bx, by + i * 54, "tiles", Controller.this));
						}
					}
					...
					Rectangle r1 = new Rectangle(bx - 5, by - 5, width * 50 + (width - 1) * 4 + 10,
							height * 50 + (height - 1) * 4 + 10);
						...
						public void handle(ActionEvent arg0) {
							...
							for (tile N : destination) {
								...
								for (int a = 0; a < path.size() - 1; a++) {
									TranslateTransition animation = new TranslateTransition(Duration.seconds(0.3),
											Agent);
										...
										pause.setOnFinished(new EventHandler<ActionEvent>() {
											...
											public void handle(ActionEvent arg0) {
												for (tile p : animations.get(index)) {
													if (p.isDirty()) {
														p.setStyle(
																"-fx-background-image:url('dust.png'); -fx-background-color: orange");
													} else {
									...
									if (a == path.size() - 2) {
										animation.setOnFinished(new EventHandler<ActionEvent>() {
											...
											public void handle(ActionEvent arg0) {
												next.setStyle(
														"-fx-background-image: none; -fx-background-color:orange");
												next.setDirty(false);
												for (tile p : animations.get(index)) {
													if (p.isDirty()) {
														p.setStyle(
																"-fx-background-image:url('dust.png'); -fx-background-color: white");
													} else {
				...
				public void handle(ActionEvent arg0) {
					if (algoIndex < arr.size()) {
						if (arr.get(algoIndex).isDirty()) {
							arr.get(algoIndex)
									.setStyle("-fx-background-image:url('dust.png'); -fx-background-color: cyan");
						} else {
